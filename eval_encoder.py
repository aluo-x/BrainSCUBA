...
def test_net(rank, other_args):
    ...
    losses = 0.0
    offset = 0.0
    print("Saving to {}".format("/ROOT/results/eval/{}.h5py".format(cur_args.exp_name)))
    file = h5py.File("./results/eval/{}.h5py".format(cur_args.exp_name), 'w')
    with torch.no_grad():
        ...
        for data_stuff_idx in range(len(dataset.testing_set)):
            ...
            predicted = ddp_projector(features.float()).reshape(-1)
